...
function broadcastPortal(portal:string|null) {
 window.dispatchEvent(new CustomEvent('hoj-portal',{detail:portal}));
 window.dispatchEvent(new CustomEvent('hustoj-portal',{detail:portal}));
}
   ...
   {tab==='insights' && (
    <div className="space-y-6">
     ...
     {data.insights?.results && data.insights.results.length > 0 && (
      <Card>
       <CardTitle title="班级评测结果分布统计" meta="按 HOJ 实时判题结果聚合" />
